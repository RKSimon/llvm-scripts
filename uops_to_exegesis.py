...
def print_cpu_uops_yaml(cpu):
   ...
   for instrNode in root.iter('instruction'):
      ...
      if instrNode.attrib['extension'] in ['AVX512EVEX']:
        if any(x in asm for x in ['VGETEXP','VGETMANT','VRND']):
          continue
      ...
      for operandNode in instrNode.iter('operand'):
        ...
        if isevex and opwidth is not None and size == '':
          if asm.startswith('VCMP') or asm.startswith('VPCMP') or asm.startswith('VFPCLASS') or asm.startswith('VPTEST') or asm.startswith('VP2INTERSECT') or asm.startswith('VPSHUFBIT') or asm.startswith('VPMOVM2') or asm.endswith('2M'):
            opsize = int(opwidth) * broadcast_factor
            if isavx512scalar or opsize == 512:
              size = 'Z'
            elif opsize == 256:
        ...
        if first:
          if isbmi or islzcnt:
            ...
            if not isevex or not (isconflict or iscompress or isexpand or asm.startswith('VPLZCNT') or asm.startswith('VPOPCNT')):
              if not isbase and not isextract and not isconvert and not asm.startswith('KMOV') and not asm.startswith('VPMOVM2') and not asm.startswith('VREDUCEP') and not asm.endswith('2M'):
                continue
